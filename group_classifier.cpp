...
GroupClassifier::GroupClassifier(char *trained_boost_filename, RegionClassifier *character_classifier, float decision_threshold) :  character_classifier_(character_classifier), decision_threshold_(decision_threshold)
{
...
bool GroupClassifier::operator()(vector<int> *group, vector<Region> *regions)
{
	assert(group != NULL);
	assert(group->size() > 1);
	assert(regions != NULL);
...
float GroupClassifier::get_votes(vector<int> *group, vector<Region> *regions)
{
	assert(group != NULL);
	assert(group->size() > 1);
	assert(regions != NULL);
